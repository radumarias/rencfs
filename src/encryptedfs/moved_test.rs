...
async fn copy_all_file_range(fs: &EncryptedFs, src_ino: u64, src_offset: u64, dest_ino: u64, dest_offset: u64, size: usize, src_fh: u64, dest_fh: u64) {
    let mut copied = 0;
    while copied < size {
        let len = fs.copy_file_range(src_ino, src_offset + copied as u64, dest_ino, dest_offset + copied as u64, size - copied, src_fh, dest_fh).await.unwrap();
...
async fn test_write_all() {
    run_test(TestSetup { data_path: format!("{TESTS_DATA_DIR}test_write_all") }, async {
        ...
        let (fh, attr) = fs.create_nod(ROOT_INODE, &test_file, create_attr_from_type(FileType::RegularFile), false, true).await.unwrap();
        let data = "test-42";
        ...
        let (fh, attr) = fs.create_nod(ROOT_INODE, &test_file_2, create_attr_from_type(FileType::RegularFile), false, true).await.unwrap();
        let data = "test-42-37-42";
        write_all_bytes_to_fs(&fs, attr.ino, 0, data.as_bytes(), fh).await.unwrap();
        let data1 = "01";
        write_all_bytes_to_fs(&fs, attr.ino, 5, data1.as_bytes(), fh).await.unwrap();
        let data2 = "02";
        ...
        let (fh, attr) = fs.create_nod(ROOT_INODE, &test_file_3, create_attr_from_type(FileType::RegularFile), false, true).await.unwrap();
        let data = "test-42-37";
        write_all_bytes_to_fs(&fs, attr.ino, 0, data.as_bytes(), fh).await.unwrap();
        write_all_bytes_to_fs(&fs, attr.ino, 5, b"37", fh).await.unwrap();
        ...
        let buf = [0; 0];
        let fh = fs.open(attr.ino, false, true).await.unwrap();
        assert!(matches!(fs.write(ROOT_INODE, 0, &buf, fh).await, Err(FsError::InvalidInodeType)));
        assert!(matches!(fs.write(0, 0, &buf, fh).await, Err(FsError::InodeNotFound)));
        let test_dir = SecretString::from_str("test-dir").unwrap();
        let (fh, dir_attr) = fs.create_nod(ROOT_INODE, &test_dir, create_attr_from_type(FileType::Directory), false, true).await.unwrap();
        assert!(matches!(fs.write(dir_attr.ino, 0, &buf, fh).await, Err(FsError::InvalidInodeType)));
    }).await
}
...
async fn test_read() {
    run_test(TestSetup { data_path: format!("{TESTS_DATA_DIR}test_read") }, async {
        ...
        let new_data = "37";
        write_all_bytes_to_fs(&fs, attr.ino, 5, new_data.as_bytes(), fh_2).await.unwrap();
        ...
        let (fh, attr) = fs.create_nod(ROOT_INODE, &test_file_4, create_attr_from_type(FileType::RegularFile), false, true).await.unwrap();
        let data = "test-42-37";
        write_all_bytes_to_fs(&fs, attr.ino, 0, data.as_bytes(), fh).await.unwrap();
        fs.flush(fh).await.unwrap();
        fs.release(fh).await.unwrap();
        ...
        let test_dir = SecretString::from_str("test-dir").unwrap();
        let (fh, dir_attr) = fs.create_nod(ROOT_INODE, &test_dir, create_attr_from_type(FileType::Directory), true, false).await.unwrap();
...
async fn test_copy_file_range() {
    run_test(TestSetup { data_path: format!("{TESTS_DATA_DIR}test_copy_file_range") }, async {
        ...
        let (fh, attr_1) = fs.create_nod(ROOT_INODE, &test_file_1, create_attr_from_type(FileType::RegularFile), true, true).await.unwrap();
        let data = "test-42";
        write_all_bytes_to_fs(fs, attr_1.ino, 0, data.as_bytes(), fh).await.unwrap();
        fs.flush(fh).await.unwrap();
        fs.release(fh).await.unwrap();
        let fh = fs.open(attr_1.ino, true, false).await.unwrap();
        let test_file_2 = SecretString::from_str("test-file-2").unwrap();
        let (fh2, attr_2) = fs.create_nod(ROOT_INODE, &test_file_2, create_attr_from_type(FileType::RegularFile), true, true).await.unwrap();
        ...
        write_all_bytes_to_fs(&fs, attr_1.ino, 7, data_37.as_bytes(), fh).await.unwrap();
        fs.flush(fh).await.unwrap();
        fs.release(fh).await.unwrap();
        let fh = fs.open(attr_1.ino, true, false).await.unwrap();
        ...
        let fh = fs.open(attr_1.ino, true, false).await.unwrap();
        let fh_2 = fs.open(attr_2.ino, false, true).await.unwrap();
        let len = fs.copy_file_range(attr_1.ino, 42, attr_2.ino, 0, 2, fh, fh_2).await.unwrap();
        assert_eq!(len, 0);
...
async fn test_read_dir() {
    run_test(TestSetup { data_path: format!("{TESTS_DATA_DIR}test_read_dir") }, async {
        ...
        let test_file = SecretString::from_str("test-file").unwrap();
        let (_fh, file_attr) = fs.create_nod(ROOT_INODE, &test_file, create_attr_from_type(FileType::RegularFile), false, false).await.unwrap();

        let test_dir = SecretString::from_str("test-dir").unwrap();
        let (_fh, dir_attr) = fs.create_nod(ROOT_INODE, &test_dir, create_attr_from_type(FileType::Directory), false, false).await.unwrap();
        let mut entries: Vec<FsResult<DirectoryEntry>> = fs.read_dir(dir_attr.ino).await.unwrap().collect();
        ...
        let test_file_2 = SecretString::from_str("test-file-2").unwrap();
        let (_fh, file_attr) = fs.create_nod(parent, &test_file_2, create_attr_from_type(FileType::RegularFile), false, false).await.unwrap();

        let test_dir_2 = SecretString::from_str("test-dir-2").unwrap();
        let (_fh, dir_attr) = fs.create_nod(parent, &test_dir_2, create_attr_from_type(FileType::Directory), false, false).await.unwrap();
        let mut entries: Vec<FsResult<DirectoryEntry>> = fs.read_dir(dir_attr.ino).await.unwrap().collect();
        entries.sort_by(|a, b| a.as_ref().unwrap().name.expose_secret().cmp(&b.as_ref().unwrap().name.expose_secret()));
        let entries: Vec<DirectoryEntry> = entries.into_iter().map(|e| e.unwrap()).collect();
        assert_eq!(entries.len(), 2);
...
async fn test_read_dir_plus() {
    run_test(TestSetup { data_path: format!("{TESTS_DATA_DIR}test_read_dir_plus") }, async {
        ...
        let test_dir = SecretString::from_str("test-dir").unwrap();
        let (_fh, dir_attr) = fs.create_nod(ROOT_INODE, &test_dir, create_attr_from_type(FileType::Directory), false, false).await.unwrap();
        let mut entries: Vec<FsResult<DirectoryEntryPlus>> = fs.read_dir_plus(dir_attr.ino).await.unwrap().collect();
        entries.sort_by(|a, b| a.as_ref().unwrap().name.expose_secret().cmp(&b.as_ref().unwrap().name.expose_secret()));
